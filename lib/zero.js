...
Zero.prototype.publish = function (name) {
  ...
  this.pub.bind(this.settings.zmq, function () {
    console.log('zmq publish callback', arguments);
    self.pub.send(self.pack({ nodeId: self.nodeId, name: name, args: args }));
  });
...
Zero.prototype.subscribe = function (name, consumer, fn) {
  ...
  if (consumer || fn) {
    ...
    // unfortunately zmq does not emit on subscribe
